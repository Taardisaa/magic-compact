...
- 在 OpenCode 上,一次性临时分支会在摘要请求前先修剪臃肿的工具 I/O。如果修剪后的请求仍无法放进模型上下文,完整回合会被动态分批摘要;超限的多回合批次会继续递归拆分。
- OpenCode 的最终原生压缩块保持静态，只在完成时一次性写入。运行中的进度通过右上角 TUI toast 实时更新；每个已验证的批次会覆盖上一个 toast，分批或递归拆分时还会显示当前处理的回合范围。
- 所有批次成功后才会原子写回摘要。若单个回合仍无法放入上下文,插件会安全停止并明确建议使用原生 `/compact`,不会部分修改会话。
- 非空但格式错误的 XML 只会在全新的轻量会话中修复一次;提供商错误和空响应不会被当成格式问题重试。
- 摘要准备成功后,Magic Compact 会通过 OpenCode 原生 compaction transaction 提交。UI 得到一个真正的原生压缩块并支持原生 revert,原始 transcript 则保持持久化。
- 重复压缩会把上一份原生摘要与新工作合并,并执行 12,000 字符硬上限,因此活跃摘要不会无限增长。
...
- 简单 — 一个命令,零配置。
- 可逆历史 — OpenCode 保留原始 transcript,并把 Magic Compact 显示为一个支持原生 revert 的原生压缩块。
